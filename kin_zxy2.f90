...
module kin_zxy2
  ...
  subroutine MLkinetic_compact_zxy2_bisect_EKE_sinrho_rigid(nmodes,rho,ntermmax,ng_vib,ng_rot,ng_cor,npseudo,&
   ...
   integer(ik) :: info,Nterms
     ...
     ig_vib(2,6,1,:) = (/0,0,2,1,3,4/)
     ig_vib(2,6,2,:) = (/2,0,0,1,5,4/)
     ...
     ig_vib(3,6,1,:) = (/0,2,0,3,1,4/)
     ig_vib(3,6,2,:) = (/2,0,0,5,1,4/)
     ...
     ig_vib(4,6,1,:) = (/1,0,0,0,5,4/)
     ig_vib(4,6,2,:) = (/2,0,2,0,3,4/)
     ig_vib(4,6,3,:) = (/0,2,2,2,3,4/)
     ig_vib(4,6,4,:) = (/2,2,0,2,5,4/)
     ...
     ig_vib(5,6,1,:) = (/1,0,0,5,0,4/)
     ig_vib(5,6,2,:) = (/2,2,0,3,0,4/)
     ig_vib(5,6,3,:) = (/0,2,2,3,2,4/)
     ig_vib(5,6,4,:) = (/2,0,2,5,2,4/)
     ig_vib(6,2,1,:) = (/0,0,2,1,3,4/)
     ig_vib(6,2,2,:) = (/2,0,0,1,5,4/)
     ig_vib(6,3,1,:) = (/0,2,0,3,1,4/)
     ig_vib(6,3,2,:) = (/2,0,0,5,1,4/)
     ig_vib(6,4,1,:) = (/1,0,0,0,5,4/)
     ig_vib(6,4,2,:) = (/2,0,2,0,3,4/)
     ig_vib(6,4,3,:) = (/0,2,2,2,3,4/)
     ig_vib(6,4,4,:) = (/2,2,0,2,5,4/)
     ig_vib(6,5,1,:) = (/1,0,0,5,0,4/)
     ig_vib(6,5,2,:) = (/2,2,0,3,0,4/)
     ig_vib(6,5,3,:) = (/0,2,2,3,2,4/)
     ig_vib(6,5,4,:) = (/2,0,2,5,2,4/)
     ig_vib(6,6,1,:) = (/0,0,1,0,4,0/)
     ig_vib(6,6,2,:) = (/0,1,0,4,0,0/)
     ig_vib(6,6,3,:) = (/1,0,0,0,6,0/)
     ig_vib(6,6,4,:) = (/1,0,0,6,0,0/)
     ig_vib(6,6,5,:) = (/0,2,2,3,3,0/)
     ig_vib(6,6,6,:) = (/1,0,0,5,5,0/)
     ig_vib(6,6,7,:) = (/2,0,2,0,7,0/)
     ig_vib(6,6,8,:) = (/2,2,0,7,0,0/)
     ig_vib(6,6,9,:) = (/2,0,2,5,3,0/)
     ig_vib(6,6,10,:) = (/2,2,0,3,5,0/)
     ig_vib(6,6,11,:) = (/0,2,2,3,3,3/)
     ig_vib(6,6,12,:) = (/1,0,0,5,5,3/)
     ig_vib(6,6,13,:) = (/2,0,2,5,3,3/)
     ig_vib(6,6,14,:) = (/2,2,0,3,5,3/)
     ig_rot(1,1,1,:) = (/1,0,0,0,0,0/)
     ig_rot(1,3,1,:) = (/1,0,0,0,5,2/)
     ig_rot(1,3,2,:) = (/1,0,0,5,0,2/)
     ig_rot(1,3,3,:) = (/2,0,2,0,3,2/)
     ig_rot(1,3,4,:) = (/2,2,0,3,0,2/)
     ig_rot(2,2,1,:) = (/1,0,0,0,0,0/)
     ig_rot(2,3,1,:) = (/1,0,0,0,5,1/)
     ig_rot(2,3,2,:) = (/1,0,0,5,0,1/)
     ig_rot(2,3,3,:) = (/2,0,2,0,3,1/)
     ig_rot(2,3,4,:) = (/2,2,0,3,0,1/)
     ig_rot(3,1,1,:) = (/1,0,0,0,5,2/)
     ig_rot(3,1,2,:) = (/1,0,0,5,0,2/)
     ig_rot(3,1,3,:) = (/2,0,2,0,3,2/)
     ig_rot(3,1,4,:) = (/2,2,0,3,0,2/)
     ig_rot(3,2,1,:) = (/1,0,0,0,5,1/)
     ig_rot(3,2,2,:) = (/1,0,0,5,0,1/)
     ig_rot(3,2,3,:) = (/2,0,2,0,3,1/)
     ig_rot(3,2,4,:) = (/2,2,0,3,0,1/)
     ig_rot(3,3,1,:) = (/0,0,1,4,4,0/)
     ig_rot(3,3,2,:) = (/0,1,0,4,4,0/)
     ig_rot(3,3,3,:) = (/0,2,2,3,3,0/)
     ig_rot(3,3,4,:) = (/0,0,1,6,4,0/)
     ig_rot(3,3,5,:) = (/0,1,0,4,6,0/)
     ig_rot(3,3,6,:) = (/1,0,0,4,6,0/)
     ig_rot(3,3,7,:) = (/1,0,0,5,5,0/)
     ig_rot(3,3,8,:) = (/1,0,0,6,4,0/)
     ig_rot(3,3,9,:) = (/2,0,2,5,3,0/)
     ig_rot(3,3,10,:) = (/2,2,0,3,5,0/)
     ig_rot(3,3,11,:) = (/0,2,2,3,3,3/)
     ig_rot(3,3,12,:) = (/1,0,0,6,6,0/)
     ig_rot(3,3,13,:) = (/1,0,0,5,5,3/)
     ig_rot(3,3,14,:) = (/2,0,2,4,7,0/)
     ig_rot(3,3,15,:) = (/2,0,2,5,3,3/)
     ig_rot(3,3,16,:) = (/2,2,0,3,5,3/)
     ig_rot(3,3,17,:) = (/2,2,0,7,4,0/)
     ig_rot(3,3,18,:) = (/2,0,2,6,7,0/)
     ig_rot(3,3,19,:) = (/2,2,0,7,6,0/)
     ig_cor(2,1,1,:) = (/2,0,0,1,0,1/)
     ig_cor(2,2,1,:) = (/2,0,0,1,0,2/)
     ig_cor(2,3,1,:) = (/0,0,2,1,3,4/)
     ig_cor(2,3,2,:) = (/2,0,0,1,5,4/)
     ig_cor(3,1,1,:) = (/2,0,0,0,1,1/)
     ig_cor(3,2,1,:) = (/2,0,0,0,1,2/)
     ig_cor(3,3,1,:) = (/0,2,0,3,1,4/)
     ig_cor(3,3,2,:) = (/2,0,0,5,1,4/)
     ig_cor(4,1,1,:) = (/1,0,0,0,0,1/)
     ig_cor(4,1,2,:) = (/2,2,0,2,0,1/)
     ig_cor(4,2,1,:) = (/1,0,0,0,0,2/)
     ig_cor(4,2,2,:) = (/2,2,0,2,0,2/)
     ig_cor(4,3,1,:) = (/1,0,0,0,5,4/)
     ig_cor(4,3,2,:) = (/2,0,2,0,3,4/)
     ig_cor(4,3,3,:) = (/0,2,2,2,3,4/)
     ig_cor(4,3,4,:) = (/2,2,0,2,5,4/)
     ig_cor(5,1,1,:) = (/1,0,0,0,0,1/)
     ig_cor(5,1,2,:) = (/2,0,2,0,2,1/)
     ig_cor(5,2,1,:) = (/1,0,0,0,0,2/)
     ig_cor(5,2,2,:) = (/2,0,2,0,2,2/)
     ig_cor(5,3,1,:) = (/1,0,0,5,0,4/)
     ig_cor(5,3,2,:) = (/2,2,0,3,0,4/)
     ig_cor(5,3,3,:) = (/0,2,2,3,2,4/)
     ig_cor(5,3,4,:) = (/2,0,2,5,2,4/)
     ig_cor(6,1,1,:) = (/1,0,0,0,5,2/)
     ig_cor(6,1,2,:) = (/1,0,0,5,0,2/)
     ig_cor(6,1,3,:) = (/2,0,2,0,3,2/)
     ig_cor(6,1,4,:) = (/2,2,0,3,0,2/)
     ig_cor(6,2,1,:) = (/1,0,0,0,5,1/)
     ig_cor(6,2,2,:) = (/1,0,0,5,0,1/)
     ig_cor(6,2,3,:) = (/2,0,2,0,3,1/)
     ig_cor(6,2,4,:) = (/2,2,0,3,0,1/)
     ig_cor(6,3,1,:) = (/0,0,1,0,4,0/)
     ig_cor(6,3,2,:) = (/0,1,0,4,0,0/)
     ig_cor(6,3,3,:) = (/1,0,0,0,6,0/)
     ig_cor(6,3,4,:) = (/1,0,0,6,0,0/)
     ig_cor(6,3,5,:) = (/2,0,2,0,7,0/)
     ig_cor(6,3,6,:) = (/2,2,0,7,0,0/)


     ipseudo(1,:) = (/0,0,1,0,0,0/)
     ipseudo(2,:) = (/0,1,0,0,0,0/)
     ipseudo(3,:) = (/1,0,0,0,0,0/)
     ipseudo(4,:) = (/0,0,1,0,4,0/)
     ipseudo(5,:) = (/0,1,0,4,0,0/)
     ipseudo(6,:) = (/0,2,2,1,1,0/)
     ipseudo(7,:) = (/0,0,1,0,6,0/)
     ipseudo(8,:) = (/0,1,0,6,0,0/)
     ipseudo(9,:) = (/1,0,0,0,6,0/)
     ipseudo(10,:) = (/1,0,0,6,0,0/)
     ipseudo(11,:) = (/0,2,2,1,3,0/)
     ipseudo(12,:) = (/0,2,2,2,2,0/)
     ipseudo(13,:) = (/0,2,2,3,1,0/)
     ipseudo(14,:) = (/0,2,2,1,1,3/)
     ipseudo(15,:) = (/0,2,2,3,3,0/)
     ipseudo(16,:) = (/2,0,2,5,1,0/)
     ipseudo(17,:) = (/2,2,0,1,5,0/)
     ipseudo(18,:) = (/0,2,2,1,3,3/)
     ipseudo(19,:) = (/0,2,2,3,1,3/)
     ipseudo(20,:) = (/1,0,0,5,5,0/)
     ipseudo(21,:) = (/2,0,2,5,3,0/)
     ipseudo(22,:) = (/2,2,0,3,5,0/)
     ipseudo(23,:) = (/0,2,2,3,3,3/)
     ipseudo(24,:) = (/2,0,2,5,1,3/)
     ipseudo(25,:) = (/2,2,0,1,5,3/)
     ipseudo(26,:) = (/1,0,0,5,5,3/)
     ipseudo(27,:) = (/2,0,2,5,3,3/)
     ipseudo(28,:) = (/2,2,0,3,5,3/)
     ...
     g_vib(4,4,3) =  -2./mX
     g_vib(4,5,1) =  -1.*(mX+mZ)/mX/mZ
     g_vib(4,5,2) =  2.*(mX+mZ)/mX/mZ
     g_vib(4,5,3) =  1/mX
     g_vib(4,5,4) =  1/mX
     g_vib(4,5,5) =  1/mX
     g_vib(4,5,6) =  -1./mX
     ...
     g_vib(5,2,5) =  2./mX
     g_vib(5,3,1) =  -1./mX
     g_vib(5,4,1) =  -1.*(mX+mZ)/mX/mZ
     g_vib(5,4,2) =  2.*(mX+mZ)/mZ/mX
     g_vib(5,4,3) =  1/mX
     g_vib(5,4,4) =  1/mX
     g_vib(5,4,5) =  1/mX
     g_vib(5,4,6) =  -1./mX
     ...
     g_vib(6,4,3) =  -1./mX
     g_vib(6,4,4) =  1/mX
     g_vib(6,5,1) =  -1.*(mX+mZ)/mZ/mX
     g_vib(6,5,2) =  1/mX
     g_vib(6,5,3) =  -1./mX
     g_vib(6,5,4) =  1/mX
     g_vib(6,6,1) =  (mY+mX)/mX/mY
     g_vib(6,6,2) =  (mY+mX)/mX/mY
     g_vib(6,6,3) =  (mZ+mX)/mX/mZ
     g_vib(6,6,4) =  (mZ+mX)/mX/mZ
     g_vib(6,6,5) =  2./mX
     g_vib(6,6,6) =  2.*(mX+mZ)/mX/mZ
     g_vib(6,6,7) =  -2./mX
     g_vib(6,6,8) =  -2./mX
     g_vib(6,6,9) =  -2./mX
     g_vib(6,6,10) =  -2./mX
     ...
     g_rot(1,3,1) =  .500*(mX+mZ)/mX/mZ
     g_rot(1,3,2) =  .500*(mX+mZ)/mX/mZ
     g_rot(1,3,3) =  -.500/mX
     g_rot(1,3,4) =  -.500/mX
     ...
     g_rot(2,3,3) =  .500/mX
     g_rot(2,3,4) =  -.500/mX
     g_rot(3,1,1) =  .500*(mX+mZ)/mX/mZ
     g_rot(3,1,2) =  .500*(mX+mZ)/mX/mZ
     g_rot(3,1,3) =  -.500/mX
     g_rot(3,1,4) =  -.500/mX
     ...
     g_cor(5,2,2) =  1/mX
     g_cor(5,3,1) =  -.500*(mX+mZ)/mZ/mX
     g_cor(5,3,2) =  .500/mX
     g_cor(5,3,3) =  -.500/mX
     g_cor(5,3,4) =  .500/mX
     g_cor(6,1,1) =  -1.*(mX+mZ)/mX/mZ
     g_cor(6,1,2) =  (mX+mZ)/mX/mZ
     g_cor(6,1,3) =  1/mX
     g_cor(6,1,4) =  -1./mX
     g_cor(6,2,1) =  (mX+mZ)/mX/mZ
     g_cor(6,2,2) =  (mX+mZ)/mX/mZ
     g_cor(6,2,3) =  -1./mX
     g_cor(6,2,4) =  -1./mX
     g_cor(6,3,1) =  -.500*(mX+mY)/mY/mX
     g_cor(6,3,2) =  .500*(mX+mY)/mY/mX
     g_cor(6,3,3) =  -.500*(mZ+mX)/mX/mZ
     g_cor(6,3,4) =  .500*(mZ+mX)/mX/mZ
     g_cor(6,3,5) =  1/mX
     g_cor(6,3,6) =  -1./mX
     pseudo(1) =  -.375*(mY+mX)/mY/mX
     pseudo(2) =  -.375*(mY+mX)/mY/mX
     pseudo(3) =  -.500*(mX+mZ)/mX/mZ
     pseudo(4) =  .125*(mX+mY)/mX/mY
     pseudo(5) =  .125*(mX+mY)/mX/mY
     pseudo(6) =  .250/mX
     pseudo(7) =  -.250*(mX+mY)/mY/mX
     pseudo(8) =  -.250*(mX+mY)/mY/mX
     pseudo(9) =  -.125*(mZ+mX)/mX/mZ
     pseudo(10) =  -.125*(mZ+mX)/mX/mZ
     pseudo(11) =  .250/mX
     pseudo(12) =  -.250/mX
     pseudo(13) =  .250/mX
     pseudo(14) =  -.500/mX

...
contains
 ...
 subroutine sf_fitting(Jval)
      ...
      character(len=cl) :: nmodes_s, nclasses_s, ncoords_s, ndigits_s
       !
       ...
       if (parmax>molec%parmax) then 
         !
         write(out,"('fitting: potential and fitting parameters do not agree (parmax>molec%parmax):',2(1x,i0))") parmax,molec%parmax
       ...
       outer_loop: do while (still_run)  
          ...
          do while( fititer.le.itmax .and. stadev.ge.fitting%target_rms.and. stability.ge.stab_best)
            ...
            rewind(abinitunit)
            !
            write(ncoords_s,'(i0)') ncoords
            write(abinitunit,"(1h1,5x,12('*'),' ab initio points ', 12('*')// 4x,"//ncoords_s//"(7x),'zero',7x, &
            ...
            do i=1,pot_npts
               ...
               !
               write (abinitunit,"("//ncoords_s//"(2x,f18.9),3(x,g18.10),x,e12.4)")& 
                      local(:,i), &
                      pot_values(i),v, &
            ...
            if (all(extF%term(:,1,1)==-1)) then
               ...
               !
               do i=1,parmax
                  write (out,"(a8,4x,*(i3),1x,i2,e22.14)") nampar(i),(extF%term(l,1,i),l=1,Ncoords),ivar(i),potparam(i)
               enddo
               !
               write(out,"(/'Potential parameters:')")
               !
               do i=1,parmax
                 write (out,"(a8,4x,*(i3),1x,i2,e22.14)") nampar(i),(extF%term(l,1,i),l=1,Ncoords),ivar(i),molec%force(i)+potparam(i)
               enddo
               !
            endif 
            !
            ...
            if (itmax.ne.0) then
              ...
              do i=1,parmax
                if (ivar(i) .ne. 0) then
                   ...
                   !
                   write(ndigits_s,'(i0)') ndigits
                   write (out,"(a8,i4,2x,f22."//ndigits_s//",'(',i14,')')") nampar(i),ivar(i),potparam(i),nint(conf_int)
                else 
                   ndigits =2
                   if (potparam(i).ne.0.0) ndigits = 8
                   !
                   write(ndigits_s,'(i0)') ndigits
                   write (out,"(a8,i4,2x,f22."//ndigits_s//")") nampar(i),ivar(i),potparam(i)
                   !
                endif
              enddo  ! --- i
              !
...
6553   format(/3X,88('-')/'   |  Iter  | Points | Params |    Deviat     |',&
 ...
 subroutine bandcentres_fitting(Jval)
      ...
      character(len=1)   :: rng
      character(len=1),allocatable  :: mark(:)
      character(len=cl) :: nmodes_s, nclasses_s, ndigits_s
       !
       if (job%verbose>=2) write(out,"(/'The least-squares fitting ...')")
       !
       ...
       do i = 1,j0fit%Nenergies
         ...
         !
         write(nampar(i),"(*(i3))") j0fit%obs(i)%quanta(1:nmodes)
         !
       enddo
       !
       forall(i=1:en_npts) wtall(i) = fitting%obs(i)%weight
          ...
          do while( fititer.le.itmax .and. stadev.ge.fitting%target_rms.and. stability.ge.stab_best)
            ...
            do i=1,j0fit%Nenergies
              ...
              if (iener<=en_npts) then 
                ...
                !
                write(out,"(3i5,2x,&
                   f18.10,2x,"//nmodes_s//"(1x, i3),f8.2,1x,'(',"//nmodes_s//"(1x, i3),')',2x,'(',"//nmodes_s//"(1x, i3),')',f12.4,2x,f12.4)") &
                ...
                !
                write(out,"(3i5,2x,&
                   f18.10,2x,"//nmodes_s//"(1x, i3),f8.2,1x,'(',"//nmodes_s//"(1x, i3),')',2x,'(',"//nmodes_s//"(4x),')',f12.4)") &
                   Jrot,j0fit%obs(i)%symmetry,j0fit%obs(i)%N,&
                   xparam(i),j0fit%obs(i)%quanta(1:nmodes),real(ivar(i),rk), &
                   eigen(jlevel)%quanta(1:nmodes),&
                   ener_j0(isym,ilevel)
                ...
                !
                write(out,"(3i5,2x,&
                   f18.10,2x,"//nmodes_s//"(1x, i3),f8.2,1x,'(',"//nmodes_s//"(4x),')',2x,'(',"//nmodes_s//"(4x),')')") &
                   Jrot,j0fit%obs(i)%symmetry,j0fit%obs(i)%N,&
                   xparam(i),j0fit%obs(i)%quanta(1:nmodes),real(ivar(i),rk)
            ...
            if (itmax.ne.0) then
              ...
              do i=1,parmax
                if (ivar(i) .ne. 0) then
                   ...
                   !
                   write (ndigits_s,'(i0)') ndigits
                   write (out,"(a8,i4,2x,f22."//ndigits_s//",'(',i14,')')") nampar(i),ivar(i),xparam(i),nint(conf_int)
                else 
                   ndigits =2
                   if (xparam(i).ne.0.0) ndigits = 8
                   !
                   write (ndigits_s,'(i0)') ndigits
                   write (out,"(a8,i4,2x,f22."//ndigits_s//")") nampar(i),ivar(i),xparam(i)
                   !
                endif
              enddo  ! --- i
              !
     ...
     if (do_calc) then 
       ...
       do jind = 1, nJ
          ...
          do isym= 1,sym%Nrepresen
            ...
            !
            if (job%verbose>=5) write (out,"('matsize,j0ener_fit_plus = ',2(i0,1x))") matsize,j0ener_fit_plus
            !
            ...
            elseif (trim(fitting%method)=='VERYFAST') then 
              ...
              do ientry = 1,Nentries
                 !
                 if (job%verbose>=5.and.mod(ientry,Nentries/50).ne.0) write(out,"('ientry = ',i0)") ientry
                 !
                 ilevel = fit(isym,jind)%ilevel(ientry)
                 !
                 irec = eigen(ilevel)%irec(1)

...
module pot_ch3oh
  ...
  implicit none
  !
  public MLpoten_ch3oh_ref,ML_CH3OH_MEP_geometry,MLpoten_ch3oh_sym,MLpoten_ch3oh_sym_IV,MLpoten_ch3oh_sym_V
  !
  private
  !
...
function MLpoten_ch3oh_sym(ncoords, natoms, local, xyz, force) result(f)
 ...
 function MLpoten_ch3oh_sym_IV(ncoords, natoms, local, xyz, force) result(f)
  ...
  case default
    ...
    stop 'MLpoten_ch3oh_sym error error: bad coordinate type'
    !
  case('R-ALPHA-THETA-TAU','R-ALPHA-THETA-S-TAU','DR-DALPHA-S-TAU','R-ALPHA132-THETA-S-TAU')
    !
    rCOe      = force(1)
    rOHe      = force(2)
    rH1e      = force(3)
  ...
  !
  call ML_symmetry_transformation_CH3OH_IV(nsym,xi,chi,ndeg)
  !
  do iparam = nparam_eq+1,parmax
    !
    do_cos = .true.
  ...
  enddo   
  !
end function  MLpoten_ch3oh_sym_IV
 
 
  function MLpoten_ch3oh_sym_V(ncoords, natoms, local, xyz, force) result(f)
  ...
  !
  real(ark) :: rCO,ROH,RCOe,ROHe,alpha_COHe,alpha_OCHe,deltae,RH1,RH2,RH3,RH1E
  real(ark) :: b1,b3,b4,b5,theta1,theta2,theta3,theta12,theta23,theta13,a1,a2,tau,t1,t2,t3
  !
  nparam_eq = 8
  deg = pi/180.0_ark
  !
  ...
  case('R-ALPHA-THETA-TAU','R-ALPHA-THETA-S-TAU','DR-DALPHA-S-TAU')
    ...
    xi(9) = local(9)- alpha_OCHe
    !
    t1 = mod(local(10),2.0_ark*pi)
    t2 = mod(local(11),2.0_ark*pi)
    t3 = mod(local(12),2.0_ark*pi)
    !
    !theta12=t2-t1
    !theta23=t3-t2
    !theta13 = mod(2.0_ark*pi-theta12-theta23,2.0_ark*pi)
    ...
    if (t1>2.0_ark*pi+sqrt(small_)) then
      t1 = t1 - 2.0_ark*pi
    endif
    !
  ...
  !
  call ML_symmetry_transformation_CH3OH_V(nsym,xi,chi,ndeg)
  !
  do iparam = nparam_eq+1,parmax
    !
    do_cos = .true.
  ...
  enddo   
  !
end function  MLpoten_ch3oh_sym_V
...
subroutine ML_symmetry_transformation_CH3OH_III(nsym,src,dst,ndeg)
    implicit none
    !
    integer(ik),intent(in)    :: nsym,ndeg  ! group operation  
    real(ark),intent(in)      :: src(1:ndeg)
    ...
    integer(rk)       :: ioper
    !
    if (verbose>=6) write(out,"('ML_symmetry_transformation_CH3OH_III/start')")
    !
    a = 0.5_ark ; b = 0.5_ark*sqrt(3.0_ark) ; e = 1.0_ark ; o = 0.0_ark
    !
    repres = 0
    ...
    dst(12,6) =-src(12)-2.0_ark*pi/3.0_ark
    !
    if (verbose>=6) write(out,"('ML_symmetry_transformation_CH3OH_III/end')")
    !
  end subroutine ML_symmetry_transformation_CH3OH_III


subroutine ML_symmetry_transformation_CH3OH_IV(nsym,src,dst,ndeg)
    implicit none
    !
    integer(ik),intent(in)    :: nsym,ndeg  ! group operation  
    real(ark),intent(in)      :: src(1:ndeg)
    ...
    if (verbose>=6) write(out,"('ML_symmetry_transformation_CH3OH_II/end')")
    !
  end subroutine ML_symmetry_transformation_CH3OH_IV
  
  subroutine ML_symmetry_transformation_CH3OH_V(nsym,src,dst,ndeg)
    implicit none
    !
    integer(ik),intent(in)    :: nsym,ndeg  ! group operation  
    real(ark),intent(in)      :: src(1:ndeg)
    ...
    if (verbose>=6) write(out,"('ML_symmetry_transformation_CH3OH_II/end')")
    !
  end subroutine ML_symmetry_transformation_CH3OH_V
